import { Vehicle, User, Transaction, DashboardStats } from '../types';

export const INITIAL_USERS: User[] = [
  {
    id: 'usr_001',
    fullName: 'Sarah Jenkins',
    email: 'sarah.j@example.com',
    phoneNumber: '+1-555-0192',
    role: 'customer',
    avatarUrl: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&auto=format&fit=crop&q=80',
    verificationStatus: 'verified',
    driverLicenseNumber: 'DL-98472910-A',
    idCardNumber: 'ID-7728109',
    membershipTier: 'premium',
    rating: 4.9,
    totalRentals: 14,
    createdAt: '2026-01-15T08:30:00Z',
  },
  {
    id: 'usr_002',
    fullName: 'Marcus Vance',
    email: 'marcus.vance@example.com',
    phoneNumber: '+1-555-0144',
    role: 'customer',
    avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80',
    verificationStatus: 'pending',
    driverLicenseNumber: 'DL-11827402-B',
    idCardNumber: 'ID-9923841',
    membershipTier: 'standard',
    rating: 4.5,
    totalRentals: 2,
    createdAt: '2026-02-10T11:20:00Z',
  },
  {
    id: 'usr_003',
    fullName: 'Elena Rostova',
    email: 'elena.r@example.com',
    phoneNumber: '+1-555-0188',
    role: 'customer',
    avatarUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80',
    verificationStatus: 'verified',
    driverLicenseNumber: 'DL-66382910-C',
    idCardNumber: 'ID-3341829',
    membershipTier: 'vip',
    rating: 5.0,
    totalRentals: 28,
    createdAt: '2025-11-04T14:15:00Z',
  },
  {
    id: 'usr_004',
    fullName: 'David Thorne (Lead Admin)',
    email: 'admin.david@vehiclelending.com',
    phoneNumber: '+1-555-0100',
    role: 'admin',
    avatarUrl: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80',
    verificationStatus: 'verified',
    driverLicenseNumber: 'DL-00019283-A',
    idCardNumber: 'ID-1000001',
    membershipTier: 'vip',
    rating: 5.0,
    totalRentals: 0,
    createdAt: '2025-01-01T00:00:00Z',
  },
  {
    id: 'usr_005',
    fullName: 'Thomas Briggs (Hub Worker)',
    email: 'worker.thomas@vehiclelending.com',
    phoneNumber: '+1-555-0177',
    role: 'worker',
    avatarUrl: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&auto=format&fit=crop&q=80',
    verificationStatus: 'verified',
    driverLicenseNumber: 'DL-55291823-W',
    idCardNumber: 'ID-5000021',
    membershipTier: 'standard',
    rating: 4.8,
    totalRentals: 0,
    createdAt: '2025-03-12T09:00:00Z',
  },
  {
    id: 'usr_006',
    fullName: 'Aisha Patel',
    email: 'aisha.patel@example.com',
    phoneNumber: '+1-555-0199',
    role: 'customer',
    avatarUrl: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=150&auto=format&fit=crop&q=80',
    verificationStatus: 'pending',
    driverLicenseNumber: 'DL-77382910-K',
    idCardNumber: 'ID-4491028',
    membershipTier: 'standard',
    rating: 4.7,
    totalRentals: 1,
    createdAt: '2026-02-20T16:45:00Z',
  }
];

export const INITIAL_VEHICLES: Vehicle[] = [
  {
    id: 'veh_001',
    name: 'Tesla Model 3 Long Range',
    brand: 'Tesla',
    model: 'Model 3 LR',
    type: 'car',
    licensePlate: 'EV-849-TX',
    serialNumber: 'TSLA3-998201-2025',
    status: 'available',
    hourlyRate: 18.5,
    dailyRate: 95.0,
    fuelOrBatteryLevel: 92,
    locationHub: 'Downtown Central Station',
    imageUrl: 'https://images.unsplash.com/photo-1536700503339-1e4b06520771?w=600&auto=format&fit=crop&q=80',
    year: 2025,
    transmission: 'automatic',
    seats: 5,
    conditionNotes: 'Pristine condition. Battery charged to 92%. Tires inspected.',
    lastMaintainedDate: '2026-02-18',
    createdAt: '2025-06-10T10:00:00Z',
  },
  {
    id: 'veh_002',
    name: 'Toyota RAV4 Hybrid',
    brand: 'Toyota',
    model: 'RAV4 AWD',
    type: 'car',
    licensePlate: 'HY-204-CA',
    serialNumber: 'TOY-RAV-448201',
    status: 'rented',
    hourlyRate: 14.0,
    dailyRate: 75.0,
    fuelOrBatteryLevel: 78,
    locationHub: 'Northside Metro Hub',
    imageUrl: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?w=600&auto=format&fit=crop&q=80',
    year: 2024,
    transmission: 'automatic',
    seats: 5,
    conditionNotes: 'Currently on active 24h loan with Sarah Jenkins.',
    lastMaintainedDate: '2026-02-10',
    createdAt: '2025-04-14T09:30:00Z',
  },
  {
    id: 'veh_003',
    name: 'Hyundai Ioniq 5 EV',
    brand: 'Hyundai',
    model: 'Ioniq 5 Ultimate',
    type: 'car',
    licensePlate: 'EL-512-NV',
    serialNumber: 'HYU-ION-558291',
    status: 'maintenance',
    hourlyRate: 16.0,
    dailyRate: 85.0,
    fuelOrBatteryLevel: 35,
    locationHub: 'Airport Terminus Hub',
    imageUrl: 'https://images.unsplash.com/photo-1503376780353-7e6692767b70?w=600&auto=format&fit=crop&q=80',
    year: 2025,
    transmission: 'automatic',
    seats: 5,
    conditionNotes: 'Scheduled brake pad inspection and 40kW fast charge in progress.',
    lastMaintainedDate: '2026-02-23',
    createdAt: '2025-07-20T13:00:00Z',
  },
  {
    id: 'veh_004',
    name: 'Honda CB500X Adventure',
    brand: 'Honda',
    model: 'CB500X',
    type: 'bike',
    licensePlate: 'BK-902-NY',
    serialNumber: 'HON-CB5-102948',
    status: 'available',
    hourlyRate: 8.5,
    dailyRate: 45.0,
    fuelOrBatteryLevel: 85,
    locationHub: 'Downtown Central Station',
    imageUrl: 'https://images.unsplash.com/photo-1558981403-c5f9899a28bc?w=600&auto=format&fit=crop&q=80',
    year: 2024,
    transmission: 'manual',
    seats: 2,
    conditionNotes: 'Helmet and safety vest provided in rear top case.',
    lastMaintainedDate: '2026-02-15',
    createdAt: '2025-08-01T11:00:00Z',
  },
  {
    id: 'veh_005',
    name: 'Yamaha NMAX 155cc Scooter',
    brand: 'Yamaha',
    model: 'NMAX 155 ABS',
    type: 'bike',
    licensePlate: 'SC-331-FL',
    serialNumber: 'YAM-NM1-884920',
    status: 'available',
    hourlyRate: 6.0,
    dailyRate: 32.0,
    fuelOrBatteryLevel: 95,
    locationHub: 'West End District Hub',
    imageUrl: 'https://images.unsplash.com/photo-1568772585407-9361f9bf3a87?w=600&auto=format&fit=crop&q=80',
    year: 2025,
    transmission: 'automatic',
    seats: 2,
    conditionNotes: 'Full tank, agile commuter vehicle with phone mount.',
    lastMaintainedDate: '2026-02-21',
    createdAt: '2025-09-12T14:30:00Z',
  },
  {
    id: 'veh_006',
    name: 'Zero DSR Electric Dual-Sport',
    brand: 'Zero Motorcycles',
    model: 'DSR ZF14.4',
    type: 'bike',
    licensePlate: 'EB-771-WA',
    serialNumber: 'ZER-DSR-449102',
    status: 'reserved',
    hourlyRate: 12.0,
    dailyRate: 65.0,
    fuelOrBatteryLevel: 90,
    locationHub: 'Northside Metro Hub',
    imageUrl: 'https://images.unsplash.com/photo-1599819811279-d5ad9cccf838?w=600&auto=format&fit=crop&q=80',
    year: 2025,
    transmission: 'automatic',
    seats: 2,
    conditionNotes: 'Reserved for Marcus Vance verification request.',
    lastMaintainedDate: '2026-02-19',
    createdAt: '2025-10-05T08:00:00Z',
  },
  {
    id: 'veh_007',
    name: 'Trek Allant+ 8 Electric City Bike',
    brand: 'Trek',
    model: 'Allant+ 8 e-Bike',
    type: 'bicycle',
    licensePlate: 'N/A (Bicycle)',
    serialNumber: 'TRK-EBIKE-88392',
    status: 'available',
    hourlyRate: 3.5,
    dailyRate: 20.0,
    fuelOrBatteryLevel: 88,
    locationHub: 'Downtown Central Station',
    imageUrl: 'https://images.unsplash.com/photo-1485965120184-e220f721d03e?w=600&auto=format&fit=crop&q=80',
    year: 2025,
    transmission: 'n/a',
    seats: 1,
    conditionNotes: 'Bosch smart motor, hydraulic disc brakes, puncture-resistant tires.',
    lastMaintainedDate: '2026-02-22',
    createdAt: '2025-05-18T10:15:00Z',
  },
  {
    id: 'veh_008',
    name: 'Specialized Sirrus X 4.0 Gravel',
    brand: 'Specialized',
    model: 'Sirrus X 4.0',
    type: 'bicycle',
    licensePlate: 'N/A (Bicycle)',
    serialNumber: 'SPC-SRX-102938',
    status: 'available',
    hourlyRate: 2.5,
    dailyRate: 15.0,
    fuelOrBatteryLevel: 100,
    locationHub: 'West End District Hub',
    imageUrl: 'https://images.unsplash.com/photo-1532298229144-0ec0c57515c7?w=600&auto=format&fit=crop&q=80',
    year: 2024,
    transmission: 'manual',
    seats: 1,
    conditionNotes: 'Lightweight alloy frame, Shimano 1x11 gearing, includes high-security U-lock.',
    lastMaintainedDate: '2026-02-14',
    createdAt: '2025-06-02T15:00:00Z',
  },
  {
    id: 'veh_009',
    name: 'Giant FastRoad E+ EX Pro',
    brand: 'Giant',
    model: 'FastRoad E+',
    type: 'bicycle',
    licensePlate: 'N/A (Bicycle)',
    serialNumber: 'GNT-FST-552910',
    status: 'maintenance',
    hourlyRate: 3.5,
    dailyRate: 22.0,
    fuelOrBatteryLevel: 45,
    locationHub: 'Northside Metro Hub',
    imageUrl: 'https://images.unsplash.com/photo-1576435728678-68d0fbf94e91?w=600&auto=format&fit=crop&q=80',
    year: 2025,
    transmission: 'n/a',
    seats: 1,
    conditionNotes: 'Chain tension adjustment and rear spoke tightening scheduled.',
    lastMaintainedDate: '2026-02-24',
    createdAt: '2025-07-11T12:00:00Z',
  }
];

export const INITIAL_TRANSACTIONS: Transaction[] = [
  {
    id: 'tx_1001',
    userId: 'usr_001',
    vehicleId: 'veh_002',
    userName: 'Sarah Jenkins',
    vehicleName: 'Toyota RAV4 Hybrid',
    vehicleType: 'car',
    licensePlate: 'HY-204-CA',
    startDateTime: '2026-02-24T08:00:00Z',
    endDateTime: '2026-02-25T08:00:00Z',
    durationHours: 24,
    totalCost: 75.0,
    status: 'active',
    paymentStatus: 'paid',
    paymentMethod: 'credit_card',
    pickupHub: 'Downtown Central Station',
    dropoffHub: 'Northside Metro Hub',
    notes: 'Verified by David Thorne. Customer picked up keys at kiosk.',
    reviewedByWorkerId: 'usr_005',
    approvedByAdminId: 'usr_004',
    createdAt: '2026-02-23T19:20:00Z',
    updatedAt: '2026-02-24T08:05:00Z',
  },
  {
    id: 'tx_1002',
    userId: 'usr_002',
    vehicleId: 'veh_006',
    userName: 'Marcus Vance',
    vehicleName: 'Zero DSR Electric Dual-Sport',
    vehicleType: 'bike',
    licensePlate: 'EB-771-WA',
    startDateTime: '2026-02-25T10:00:00Z',
    endDateTime: '2026-02-25T16:00:00Z',
    durationHours: 6,
    totalCost: 72.0,
    status: 'pending_verification',
    paymentStatus: 'pending',
    paymentMethod: 'e_wallet',
    pickupHub: 'Northside Metro Hub',
    dropoffHub: 'Northside Metro Hub',
    notes: 'Awaiting admin license verification before vehicle release.',
    createdAt: '2026-02-24T18:40:00Z',
    updatedAt: '2026-02-24T18:40:00Z',
  },
  {
    id: 'tx_1003',
    userId: 'usr_003',
    vehicleId: 'veh_007',
    userName: 'Elena Rostova',
    vehicleName: 'Trek Allant+ 8 Electric City Bike',
    vehicleType: 'bicycle',
    licensePlate: 'N/A (Bicycle)',
    startDateTime: '2026-02-24T14:00:00Z',
    endDateTime: '2026-02-24T18:00:00Z',
    durationHours: 4,
    totalCost: 14.0,
    status: 'completed',
    paymentStatus: 'paid',
    paymentMethod: 'credit_card',
    pickupHub: 'Downtown Central Station',
    dropoffHub: 'Downtown Central Station',
    notes: 'Returned on time. Battery returned at 88%. No damage recorded.',
    reviewedByWorkerId: 'usr_005',
    approvedByAdminId: 'usr_004',
    createdAt: '2026-02-24T12:10:00Z',
    updatedAt: '2026-02-24T18:02:00Z',
  },
  {
    id: 'tx_1004',
    userId: 'usr_006',
    vehicleId: 'veh_001',
    userName: 'Aisha Patel',
    vehicleName: 'Tesla Model 3 Long Range',
    vehicleType: 'car',
    licensePlate: 'EV-849-TX',
    startDateTime: '2026-02-26T09:00:00Z',
    endDateTime: '2026-02-27T09:00:00Z',
    durationHours: 24,
    totalCost: 95.0,
    status: 'pending_verification',
    paymentStatus: 'paid',
    paymentMethod: 'credit_card',
    pickupHub: 'Downtown Central Station',
    dropoffHub: 'Airport Terminus Hub',
    notes: 'New user registration. Driver license validation pending.',
    createdAt: '2026-02-24T22:15:00Z',
    updatedAt: '2026-02-24T22:15:00Z',
  }
];

export const HUBS = [
  'Downtown Central Station',
  'Northside Metro Hub',
  'West End District Hub',
  'Airport Terminus Hub',
  'University Campus Point'
];
